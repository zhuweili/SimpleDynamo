...
public class Send_backup {
    ...
    public  Send_backup (String mes, Integer portnum){
        this.mes=mes;
    ...
    public void exc() {
        ...
        SocketMap_link socket_map = new SocketMap_link();
...
//                n++;
                    ...
                    Socket socket=socket_map.socket_map1.get(portnum);
                    if (socket==null){
                        socket=new Socket("10.0.2.2", portnum * 2);
                        socket.setKeepAlive(true);
                        socket_map.socket_map1.put(portnum,socket);
                    }
                    OutputStream Send_out = socket.getOutputStream();
                    Send_out.write(msgToSend.getBytes("utf-8"));
                    //Send_out.close();
